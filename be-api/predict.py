...
# -------------------------------------------------------------
# 정사각형 패딩 (letterbox) - train.py와 완전히 동일한 함수
# -------------------------------------------------------------
...
# -------------------------------------------------------------
# 테스트할 새 이미지 전처리
# pad_to_square를 Resize 앞에 추가 (train.py의 val 전처리와 반드시 동일해야 함)
# -------------------------------------------------------------
predict_transform = transforms.Compose([
    transforms.Lambda(pad_to_square),   # 비율 왜곡 방지
...
# -------------------------------------------------------------
# 이미지 한 장 받아서 예측하는 함수 (모델을 이미 불러온 상태에서 호출)
# -------------------------------------------------------------
...
# -------------------------------------------------------------
# [신규] postEx.py가 실제로 호출하는 함수
# 체크포인트 경로 + 이미지 경로만 넘기면 모델 로딩부터 예측까지 한 번에 처리
# -------------------------------------------------------------
_model_cache = {}  # 같은 체크포인트를 요청마다 다시 로딩하면 느리므로 캐싱
...
def predict(image_path, checkpoint_path):
    ...
    model, class_names = _model_cache[checkpoint_path]
    predicted_class, confidence, _ = predict_image(image_path, model, class_names)

    return {
        "predicted_class": predicted_class,
        "confidence": confidence / 100,  # postEx.py는 0~1 사이 값을 기대함
    }


# -------------------------------------------------------------
#  !! 단독 실행 시 테스트 !!
#  (postEx.py가 import할 때는 여기 아래로 실행되지 않음)
# -------------------------------------------------------------
